...
//}
//Polya 定理
void dfs(char map[55][55],int r,int c,int x,int y,const string& d)
{
    if (d == "NORTH")
	{
        while (x > 0 && map[x-1][y] != 'X') x--;
    }
	else if (d =="SOUTH")
	{
        while (x < r-1 && map[x+1][y] != 'X') x++;
    }
	else if (d == "WEST")
	{
        while (y > 0 && map[x][y-1] != 'X') y--;
    }
	else if (d == "EAST")
	{
        while (y < c-1 && map[x][y+1] != 'X') y++;
    }
    map[x][y]='*';
}
