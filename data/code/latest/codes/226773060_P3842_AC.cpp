...
int l[maxn],r[maxn],dp[maxn][2],len[maxn];
signed main()
{
	...
	for(int i=2;i<=n;i++)
	{
		dp[i][0]=min(dp[i-1][0]+abs(r[i]-l[i-1]),dp[i-1][1]+abs(r[i-1]-r[i]))+len[i];
 		dp[i][1]=min(dp[i-1][0]+abs(l[i-1]-l[i]),dp[i-1][1]+abs(r[i-1]-l[i]))+len[i];
	}
