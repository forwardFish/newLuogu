...
int main(){
    ...
    // INIT n D m L alpha com_min beta K_max
    {
        ...
        if(sscanf(line.c_str(), "%s %d %d %lld %d %lf %lf %lf %d",
                  tag, &tr.n, &tr.D, &m, &tr.L, &tr.alpha, &tr.com_min, &tr.beta, &tr.Kmax) != 9){
            return 0;
        }
        tr.cash = m * 100LL;
        tr.pos.assign(tr.n, 0);
        tr.cost.assign(tr.n, 0);
    ...
    // read day0 exactly n lines (generally no logs here)
    for(int i=0;i<tr.n;i++){
        ...
        // seed last_close and push day0 into indicators (day=0)
        auto &s = tr.st[i];
        if(tr.cl[i]>0){
            s.have_last = true;
            s.last_close = tr.cl[i];
            s.peak_close = tr.cl[i];
        }
        tr.day = 0;
        tr.update_one(i);
    }
    ...
    // after receiving day t data, output orders for day t+1, then read day t+1 data
    while(true){
        ...
        // if already got day D, next output triggers FINISH (last execution at close)
        if(tr.day == tr.D){
            // read until FINISH
            string l2;
            while(IN.getline_str(l2)){
                if(l2.rfind("FINISH",0)==0) return 0;
                if(l2.rfind("INVALID",0)==0 || l2.rfind("ERROR",0)==0) return 0;
            }
            return 0;
        }

        // read next day market robustly
        tr.day += 1;
        int rc = read_one_day_market(tr);
        if(rc == 1) return 0;
        if(rc == -1) return 0;
        ...
        // update indicators with today's market
        for(int i=0;i<tr.n;i++){
            tr.update_one(i);
        }
    }
