...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (isDatabaseMissing(error)) return jsonResponse(await localSyncJob());
    return jsonResponse({ error: "failed to run Luogu sync", detail: formatErrorDetail(error) }, { status: 500 });
  }
}
