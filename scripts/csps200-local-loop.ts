...
async function main() {
  ...
  if (action === "benchmark") {
    ...
    return writeAndPrintState(config, "benchmark", ["pnpm csps200:plan"]);
  }
  ...
  if (action === "today") {
    ...
    return writeAndPrintState(config, "today", [
      ...
      "做完题后执行 pnpm csps200:log -- --problemPid Pxxxx --unitId UNIT --result AC --score 100 --timeMinutes 35",
    ]);
  }
  ...
  if (action === "log") {
    ...
    return writeAndPrintState(config, "log", [
      "继续完成今日剩余题目",
      "今日结束后执行 pnpm csps200:settle",
    ]);
  }

  if (action === "settle") {
    await buildTrainingSettlement(trainingOptions(config));
    return writeAndPrintState(config, "settle", ["pnpm csps200:today", "pnpm csps200:week"]);
  }
  ...
  if (action === "week") {
    ...
    return writeAndPrintState(config, "week", [
      ...
      "根据 next_week_plan.json 执行下一周训练",
    ]);
  }
...
async function initLocalLoop(options: { quiet?: boolean } = {}) {
  ...
  if (!options.quiet) {
    ...
    console.log("Next: edit data/local-loop/config.json, then run pnpm csps200:diagnose");
  }
}
...
async function normalizeToday(config: LocalLoopConfig) {
  ...
  const strategy = await readJson<JsonObject>(LOCAL_STRATEGY_PATH);
  ...
  const normalizedTasks = pickThreeTasks(problemTasks).map((task, index) => ({
    order: index + 1,
  ...
  const today = {
    ...
    todayGoal: getString(daily, "todayGoal"),
    whyToday: asArray(daily.whyToday),
...
function pickThreeTasks(tasks: JsonObject[]) {
  ...
  while (output.length < 3) {
    output.push({
      ...
      goal: output.length === 2 ? "重做本周失败题或同类迁移题。" : "从 problem_ladders 中人工选择一题补齐。",
...
function buildLogTemplate(task: JsonObject) {
  ...
  return `pnpm csps200:log -- --problemPid ${pid} --unitId UNKNOWN_UNIT --taskType ${getString(task, "type") || "PRACTICE_STANDARD"} --result AC --score 100 --timeMinutes 35 --submissionCount 1 --hintLevelUsed 0 --hasSeenSolution false --errorTypes ""`;
}
...
async function runWeek(config: LocalLoopConfig, week: number) {
  ...
  const weakMastery = arrayOfObjects(mastery.items)
    ...
    .slice(0, 8);
  const nextWeekPlan = buildNextWeekPlan(summary, weakMastery, strategy);
  const report = {
    generatedAt: new Date().toISOString(),
    week,
    targetScore: config.targetScore,
...
function buildNextWeekPlan(summary: JsonObject, weakMastery: JsonObject[], strategy: JsonObject) {
...
function renderTodayMarkdown(today: JsonObject) {
  ...
  return [
    ...
    "## 记录要求",
    "",
    "每题必须记录 result / score / timeMinutes / submissionCount / hintLevelUsed / hasSeenSolution / errorTypes / needRedo。",
    "",
  ].join("\n");
}
...
Actions:
  pnpm csps200:init
  pnpm csps200:diagnose
  pnpm csps200:benchmark
  pnpm csps200:plan
  pnpm csps200:today
  pnpm csps200:log -- --problemPid Pxxxx --unitId UNIT --result AC --score 100 --timeMinutes 35 --submissionCount 1 --hintLevelUsed 0 --hasSeenSolution false --errorTypes ""
  pnpm csps200:settle
  pnpm csps200:mock
  pnpm csps200:week -- --week 1
  pnpm csps200:verify
  pnpm csps200:all
`);
}
...
function getRecordNumber(value: unknown, key: string): Record<string, number> {
  const record = asRecord(asRecord(value)[key]);
