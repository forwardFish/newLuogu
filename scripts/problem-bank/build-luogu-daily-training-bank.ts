...
type CatalogProblem = {
  ...
  url: string;
  problemUrl: string;
  difficulty: string | DifficultyInfo | null;
...
type ModuleRule = {
  ...
  name: string;
  priority: number;
  bucket: "algorithm" | "dataStructure" | "math" | "graph" | "string" | "implementation" | "search" | "dp" | "other";
  patterns: RegExp[];
  abilityTags: string[];
  scoreFocus: string[];
  preCheck: string[];
...
const MODULE_RULES: ModuleRule[] = [
  {
    ...
    priority: 100,
    bucket: "dp",
    patterns: [/动态规划|\bDP\b|ALGO_DP|线性 DP|递推|记忆化搜索/i],
  ...
  },
  {
    ...
    priority: 90,
    bucket: "search",
    patterns: [/搜索|DFS|BFS|深度优先|广度优先|ALGO_SEARCH|SEARCH_DFS|SEARCH_BFS/i],
  ...
  },
  {
    ...
    priority: 80,
    bucket: "algorithm",
    patterns: [/贪心|GREEDY|排序|ALGO_SORTING/i],
  ...
  },
  {
    ...
    priority: 95,
    bucket: "graph",
    patterns: [/图论|最短路|拓扑排序|Floyd|Dijkstra|SPFA|GRAPH|SHORTEST_PATH|TOPOLOGICAL/i],
  ...
  },
  {
    ...
    priority: 76,
    bucket: "math",
    patterns: [/数学|gcd|最大公约数|质数|素数|逆元|组合数学|Catalan|MATH_|进制|高精度/i],
...
async function main(): Promise<void> {
  ...
  const output = {
    ...
    sourceCatalog: {
      ...
      source: getObject(catalog.source),
      note: "This file is derived from Luogu public problem metadata. It is optimized for daily CSP-J/S training selection and does not copy problem statements.",
    },
...
function extractCatalogItems(catalog: JsonObject): CatalogProblem[] {
  const rawItems = Array.isArray(catalog.items) ? catalog.items : [];
  return rawItems.map(normalizeCatalogProblem).filter((item): item is CatalogProblem => item !== null);
}

function normalizeCatalogProblem(raw: unknown): CatalogProblem | null {
  ...
  const difficultyLevel = isObject(rawDifficulty)
    ? firstNumber(rawDifficulty.level)
    : firstNumber(raw.difficultyLevel, rawDifficulty);
  ...
  return {
    ...
    problemUrl: getString(raw.problemUrl) || getString(raw.url) || `https://www.luogu.com.cn/problem/${pid}`,
    difficulty: isObject(rawDifficulty)
      ? {
          level: firstNumber(rawDifficulty.level),
          name: getString(rawDifficulty.name) || difficultyName(firstNumber(rawDifficulty.level)),
          band: getString(rawDifficulty.band) || difficultyBand(firstNumber(rawDifficulty.level)),
        }
      : getString(rawDifficulty) || null,
    difficultyLevel,
    rawLuoguTags: arrayOfStrings(raw.rawLuoguTags),
    luoguTags: arrayOfStrings(raw.luoguTags),
    luoguTagIds: arrayOfNumbers(raw.luoguTagIds),
...
function toDailyProblem(problem: CatalogProblem): DailyProblem {
  ...
  const targetTrack = inferTargetTrack(difficulty.level, contestTags, cautionTags);
  const stage = inferStage(difficulty.level, targetTrack, cautionTags);
  const trainingRoles = inferTrainingRoles(primaryModule, difficulty.level, contestTags, cautionTags);
  ...
  const recommendedMinutes = inferRecommendedMinutes(primaryModule, difficulty.level, cautionTags, text);
  const selectionScore = buildSelectionScore({ targetTrack, stage, difficultyLevel: difficulty.level, primaryModule, contestTags, cautionTags });
  ...
  return {
    ...
    url: problem.url,
    difficulty,
    rawLuoguTags: uniqueStrings([...problem.rawLuoguTags, ...problem.luoguTags]),
    luoguTagIds: uniqueNumbers(problem.luoguTagIds),
    originalTags: uniqueStrings([...problem.tags, ...Object.values(problem.categories).flat()]),
    classification: {
      targetTrack,
      stage,
      primaryModule: {
    ...
    dailyTraining: {
      recommendedUse,
      recommendedMinutes,
      repeatGapDays: inferRepeatGapDays(recommendedUse, difficulty.level),
      trainingRoles,
      scoreFocus: uniqueStrings(primaryModule.scoreFocus),
      preCheck: uniqueStrings(primaryModule.preCheck),
      postCheck: uniqueStrings(primaryModule.postCheck),
      selectionScore,
    },
    stats: problem.stats,
    source: "luogu_problem_list",
...
function buildDailySelectionPolicy(): JsonObject {
  return {
    ...
    scoreTargetMapping: {
      ...
      CSP_J_80_TO_100: ["dp_foundation", "greedy", "data_structure_basic", "graph_basic"],
      CSP_S_100_TO_140: ["dp_foundation", "search_dfs_bfs", "greedy", "binary_search", "graph_basic"],
  ...
  return [
    ...
    },
    {
      ...
      slots: [
        ...
        { slot: "core", count: 1, minutes: 35, modules: ["dp_foundation", "search_dfs_bfs", "greedy"] },
...
function buildIndexes(items: DailyProblem[]): JsonObject {
  return {
    ...
    topDailyCandidates: {
      cspJ: items
        .filter((item) => item.classification.targetTrack === "CSP_J")
        .sort((a, b) => b.dailyTraining.selectionScore.cspJ - a.dailyTraining.selectionScore.cspJ)
        .slice(0, 300)
        .map((item) => item.pid),
      cspS: items
        .filter((item) => item.classification.targetTrack === "CSP_S")
        .sort((a, b) => b.dailyTraining.selectionScore.cspS - a.dailyTraining.selectionScore.cspS)
        .slice(0, 500)
        .map((item) => item.pid),
    },
  };
...
function buildClassificationText(problem: CatalogProblem): string {
  return uniqueStrings([
    problem.title,
    ...problem.rawLuoguTags,
    ...problem.luoguTags,
    ...problem.tags,
    ...Object.values(problem.categories).flat(),
  ]).join(" ");
}

function normalizeDifficulty(problem: CatalogProblem): DifficultyInfo {
  if (isObject(problem.difficulty)) {
    return {
      level: problem.difficulty.level,
      name: problem.difficulty.name || difficultyName(problem.difficulty.level),
      band: problem.difficulty.band || difficultyBand(problem.difficulty.level),
    };
  }
  const level = problem.difficultyLevel;
  return {
    level,
    name: typeof problem.difficulty === "string" && problem.difficulty ? problem.difficulty : difficultyName(level),
...
function difficultyBand(level: number | null): string {
...
function inferCautionTags(text: string, problem: CatalogProblem): string[] {
  const tags: string[] = [];
  if (/疑似错题|暂无评定|Special Judge|OJ_SPECIAL_JUDGE|提交答案|OJ_OUTPUT_ONLY/i.test(text)) tags.push("NEED_MANUAL_REVIEW");
...
function inferExtraAbilityTags(text: string): string[] {
  const tags: string[] = [];
  if (/高精度|Big Integer/i.test(text)) tags.push("BIG_INTEGER_CARE"), tags.push("OVERFLOW_CONTROL");
...
function inferRepeatGapDays(
  recommendedUse: DailyProblem["dailyTraining"]["recommendedUse"],
  difficultyLevel: number | null,
): number[] {
...
function buildSelectionScore(params: {
  ...
  const { targetTrack, stage, difficultyLevel, primaryModule, contestTags, cautionTags } = params;
  const level = difficultyLevel ?? 2;
  const cautionPenalty = cautionTags.length > 0 ? 35 : 0;
  const contestBonus = contestTags.length > 0 ? 12 : 0;
  const coreModuleBonus = ["dp", "graph", "dataStructure", "search"].includes(primaryModule.bucket) ? 12 : 5;
  const foundationBonus = stage === "FOUNDATION" ? 8 : 0;

  const cspJ = clampScore(
    50 + (targetTrack === "CSP_J" ? 20 : 0) + (level <= 3 ? 18 : -level * 5) + coreModuleBonus + contestBonus + foundationBonus - cautionPenalty,
  );
  const cspS = clampScore(
    45 + (targetTrack === "CSP_S" ? 22 : 0) + (level >= 4 && level <= 5 ? 18 : level >= 6 ? 2 : -6) + coreModuleBonus + contestBonus - cautionPenalty,
  );
  const daily = clampScore(Math.round((cspJ + cspS) / 2 + (stage === "REVIEW_ONLY" ? -30 : 8)));
  return { cspJ, cspS, daily };
}
...
function normalizeCategories(value: unknown): Record<string, string[]> {
  ...
  for (const [key, raw] of Object.entries(value)) output[key] = arrayOfStrings(raw);
  return output;
}

function normalizeStats(value: unknown): CatalogProblem["stats"] {
  if (!isObject(value)) return { totalSubmit: null, accepted: null, passRate: null };
  return {
    totalSubmit: firstNumber(value.totalSubmit, value.submitCount),
    accepted: firstNumber(value.accepted, value.acceptedCount),
    passRate: firstNumber(value.passRate),
  };
}
