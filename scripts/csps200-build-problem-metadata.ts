...
async function main() {
  ...
  const vectorsFile = await readJson<Obj>("data/diagnosis/problem_feature_vectors.json", {});
  const timelineFile = await readJson<Obj>("data/analysis/problem_code_timeline.json", {});
  const existing = await readJson<Obj>("data/problem-metadata/problem_knowledge_map.json", {});
  const map: Record<string, MetadataItem> = {};

  for (const item of [...arrayOfObjects(timelineFile.items), ...arrayOfObjects(vectorsFile.items)]) {
    const pid = getPid(item);
    if (!pid) continue;
    map[pid] = mergeItem(map[pid], inferFromProblem(item));
  }
...
function mergeItem(a: MetadataItem | undefined, b: MetadataItem): MetadataItem {
  ...
  return {
    ...
    confidence: rankConfidence(a.confidence) >= rankConfidence(b.confidence) ? a.confidence : b.confidence,
...
function inferSlot(tags: string[], abilities: string[], difficulty: number | null, item: Obj): Slot {
  const explicit = normalizeSlot(getString(item, "cspSlot"));
  if (explicit !== "UNKNOWN") return explicit;
  const text = `${tags.join(" ")} ${abilities.join(" ")} ${strings(item.cspAbilityRoles).join(" ")}`.toUpperCase();
  ...
  if (difficulty !== null) {
    if (difficulty <= 1300) return "T1";
    if (difficulty <= 1900) return "T2";
    if (difficulty <= 2600) return "T3";
    return "T4";
  }
  return "UNKNOWN";
}
...
function inferConfidence(item: Obj, difficulty: number | null, tags: string[], slot: string): Confidence {
  let score = 0;
...
function normalizeSlot(value: string): Slot {
  const upper = value.toUpperCase();
  return ["T1", "T2", "T3", "T4"].includes(upper) ? upper as Slot : "UNKNOWN";
}
function normalizeConfidence(value: string): Confidence | null {
  const upper = value.toUpperCase();
  return ["LOW", "MEDIUM", "HIGH"].includes(upper) ? upper as Confidence : null;
}
function rankConfidence(value: Confidence) { return value === "HIGH" ? 3 : value === "MEDIUM" ? 2 : 1; }
